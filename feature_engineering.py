...
def plot_features(df, df2):
    ...
    n_cols = literal_size+1 if config.FEATURE_ALGORITHM == 'centroid' else literal_size  # centroid时每行3个子图
    ...
    for window_size in config.WINDOW_SIZES:
        ...
        if config.FEATURE_ALGORITHM == 'centroid':
            for dim in range(literal_size):  # 绘制dim_0和dim_1
                plt.subplot(n_rows, n_cols, plot_idx)
...
def calculate_features_chunk(chunk, window_size):
    ...
    for idx in range(len(chunk)):
        ...
        if config.FEATURE_ALGORITHM == 'centroid':
            ...
            top_bins = heapq.nlargest(5, bin_counts.items(), key=lambda x: x[1])
